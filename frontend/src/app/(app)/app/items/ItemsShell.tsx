...
export default function ItemsShell({ children, footer }: { children: React.ReactNode; footer: React.ReactNode }) {
  return (
    <div className="appPageContainer">
      <div className="z-10 portrait:sm:mt-6 landscape:lg:mt-6 w-full portrait:sm:max-w-125 landscape:lg:max-w-125 bg-card portrait:sm:rounded-2xl landscape:lg:rounded-2xl portrait:sm:border landscape:lg:border border-borderFaint overflow-hidden shadow-[0px_0px_12px_0px_rgba(0,0,0,0.0.08)] dark:shadow-none">
      ...
      <div className="z-10 flex-none w-full h-[var(--listButtonContainerHeight)] flex items-center justify-center bg-card portrait:sm:bg-transparent landscape:lg:bg-transparent">
        {footer}
      </div>
    </div>
  );
