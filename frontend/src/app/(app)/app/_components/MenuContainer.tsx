export default function MenuContainer({ children }: { children: React.ReactNode }) {
  const menuSize = "w-(--menuWidth) h-(--menuHeight) landscape:h-dvh portrait:w-dvw";
  const menuBg =
    "bg-card dark:bg-bgPrimary dark:portrait:bg-gradient-to-b dark:landscape:bg-gradient-to-l dark:from-card dark:to-transparent dark:to-60%";
  const menuShadow = "shadow-[0px_0px_16px_0px_rgba(0,0,0,0.1)] dark:shadow-none";

  return (
    <div
      className={`fixed z-20 portrait:bottom-0 landscape:left-0 portrait:pb-[env(safe-area-inset-bottom)] landscape:pl-[env(safe-area-inset-left)] flex items-center justify-center ${menuSize} ${menuBg} ${menuShadow}`}
    >
      <div className="landscape:w-full landscape:h-[70%] landscape:max-h-100 portrait:w-[80%] portrait:max-w-140 flex landscape:flex-col items-center justify-between">
        {children}
      </div>
    </div>
  );
}
