...
export default function Input({ inputSize = "base", variant = "primary", className = "", ...props }: InputProps) {
  ...
  const sizes = {
    xs: "h-11 desktop:h-8 px-3 desktop:px-2.5 textBase rounded-lg", // used in DetailsModal.tsx
    sm: "h-12 desktop:h-9 px-3 desktop:px-2.5 textBase rounded-lg", // not used anywhere yet
    base: "h-13 desktop:h-10 px-3.5 desktop:px-3 textBase rounded-lg",
  };
